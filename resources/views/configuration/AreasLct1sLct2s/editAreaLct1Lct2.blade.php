	...
	<div class="card">
		<div class="card-header">
			<div class="nav justify-content-center">
				<h4>Actualizar {{ $labelAreaLct1Lct2 }}</h4>
			</div>
		</div>
		<div class="card-body">
			@if($errors->any())
			...
			<form method = "POST" action="{{ route($model . '.update', $arrAreaLct1Lct2['id']) }}">
				{{ method_field('PUT') }}
				@csrf
				<div class="form-row nav justify-content-center">
					<div class="form-group col-md-8">
						<label for="name">Nombre:</label>
						<input type="text" class="form-control" name="name" id="name" placeholder="Nombre {{ $labelAreaLct1Lct2 }}" value="{{ old('name', $arrAreaLct1Lct2['name']) }}">
						@if($errors->has('name'))
				    	...
				    	<select class="form-control" id="{{ $fieldNameSup }}" name="{{ $fieldNameSup }}">
				    		@if($model == 'lct2s')
					    		@foreach($varComboSups as $varComboSup)
					      			<option value="{{ $varComboSup['id'] }}" @if($arrAreaLct1Lct2['idSup'] == $varComboSup['id']) selected @endif>
					      				{{ $varComboSup['name'] }}
					      			</option>
					      		@endforeach
				    		@else
				...
				<div class="form-row nav justify-content-center">
					<button type="submit" class="btn btn-primary">Actualizar {{ $labelAreaLct1Lct2 }}</button>
				</div>
			</form>
		</div>
