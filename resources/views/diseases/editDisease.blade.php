    ...
    <div class="card">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-end p-2">
          <h3>Actualizar Enfermedad para el Animal:
      ...
      <div class="card-body">
        ...
        <form method = "POST" action="{{ route('diseases.update', $disease->id) }}">
          ...
          <div class="form-row nav justify-content-center">
            <div class="form-group col-md-4">
                ...
                <select class="form-control" name="veterinarian_id">
                    @foreach($veterinarians as $veterinarian)
                        <option value="{{ $veterinarian->id }}" @if($disease->veterinarian_id == $veterinarian->id) selected @endif>{{ $veterinarian->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <div class="input-group date">
                    <input type="text" class="form-control datepicker" name="dateface" id="dateface" placeholder="dd/mm/aaaa" autocomplete="off" value="{{ old('dateface', $disease->review_date) }}">
            ...
            <div class="form-group col-md-4">
                ...
                <select class="form-control" name="diagnostic_id">
                    @foreach($diagnostics as $diagnostic)
                        <option value="{{ $diagnostic->id }}" @if($disease->diagnostic_id == $diagnostic->id) selected @endif>{{ $diagnostic->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <select class="form-control" name="cause_id">
                    @foreach($causes as $cause)
                        <option value="{{ $cause->id }}" @if($disease->cause_id == $cause->id) selected @endif>{{ $cause->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <select class="form-control" name="responsible_id">
                    @foreach($responsibles as $responsible)
                        <option value="{{ $responsible->id }}" @if($disease->responsible_id == $responsible->id) selected @endif>{{ $responsible->name }}</option>
                    @endforeach
                </select>
          ...
          <input type="text" name="review_date" id="review_date" hidden value="{{ old('review_date', $disease->review_date) }}">
          <!-- -->
          <div class="form-row nav justify-content-center">
